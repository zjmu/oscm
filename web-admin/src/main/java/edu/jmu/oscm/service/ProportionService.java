...
//    }
    ...
    public Map<String,Object> getListByType(List<Proportion> proportions, Boolean type) {
        ...
        for(int i = 0;i<proportions.size();i++){
            if (proportions.get(i).getAsset_or_debt()==type) {
                total = total + Float.valueOf(proportions.get(i).getReportItemInstance().getEND_VALUE());
        ...
        for (Proportion proportion : proportions) {
            if (proportion.getAsset_or_debt()==type) {
                accumProp = accumProp + Float.valueOf(proportion.getReportItemInstance().getEND_VALUE()) / total;
                proportionView = new ProportionView(proportion.getReportItemInstance().getITEM_NAME(), Float.valueOf(proportion.getReportItemInstance().getEND_VALUE()), Float.valueOf(proportion.getReportItemInstance().getEND_VALUE()) / total, accumProp);
