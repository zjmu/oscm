    ...
    public Object doIntercept(Invocation invocation) throws Throwable {
        ...
        if (!runtimeDialect.skip(ms, parameterObject, rowBounds)) {
            ...
            if (runtimeDialect.beforeCount(ms, parameterObject, rowBounds)) {
                ...
                //截取有效部分进行count操作
                if (countSql.contains(CustomDialect.SQL_SIGN)) {
                    // 截取位置 到AS limitable
                    Integer subIndex = countSql.indexOf(CustomDialect.SQL_SIGN) + CustomDialect.SQL_SIGN.length();
                    // 进行截取
                    countSql = countSql.substring(0, subIndex);

                    // 参数舍弃
                    Integer usefulParamCount = 0;
            ...
            if (runtimeDialect.beforePage(ms, parameterObject, rowBounds)) {
                ...
                //参数调度
                if (pageSql.contains(CustomDialect.SQL_SIGN)) {
                    String sqlInCount = pageSql.substring(0, pageSql.indexOf(CustomDialect.SQL_SIGN));
                    Integer count = -2;
